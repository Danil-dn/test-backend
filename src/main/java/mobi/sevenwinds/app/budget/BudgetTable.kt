...
class BudgetEntity(id: EntityID<Int>) : IntEntity(id) {
    ...
    fun toResponse(): BudgetCreateRecordParams {
        return BudgetCreateRecordParams(year, month, amount, type, author?.id?.value)
    }
    fun toBudgetAuthorResponse(): BudgetRecordResponse {
        return BudgetRecordResponse(year, month, amount, type, author?.fullName, author?.cdat.toString())
    }
}